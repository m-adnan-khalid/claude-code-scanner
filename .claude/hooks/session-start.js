  ...
  for (const file of files) {
    ...
    if (/DEVELOPING|DEV_TESTING|REVIEWING|CI_PENDING|QA_TESTING|QA_SIGNOFF|BIZ_SIGNOFF|TECH_SIGNOFF|DEPLOYING|MONITORING/.test(status)) {
      if (!activeFound) {
        ...
        if (lastActivity) {
          ...
          if (hoursStale > 6) {
            ...
            if (fs.existsSync(execDir)) {
              ...
              // Check for agent timeout checkpoints
              const timeouts = fs.readdirSync(execDir)
                .filter(f => f.startsWith(id) && f.includes('_agent_timeout_'));
              if (timeouts.length > 0) {
                const latest = timeouts[timeouts.length - 1];
                try {
                  const checkpoint = JSON.parse(fs.readFileSync(path.join(execDir, latest), 'utf8'));
                  console.log(`  AGENT TIMEOUT: ${checkpoint.agent || 'unknown'} timed out (${checkpoint.turns_used || '?'}/${checkpoint.max_turns || '?'} turns)`);
                  console.log(`    Re-invoke with narrower scope or increase maxTurns.`);
                } catch (e) {
                  console.log(`  AGENT TIMEOUT: checkpoint found at ${latest}`);
                }
              }
            }
          }
        ...
        // Loop state display + circuit breaker enforcement
        ...
        if (loopSection) {
          ...
          if (loops.length > 0) {
            ...
            // Circuit breaker detection — check if any loop hit max
            const breakerPatterns = [
              { pattern: /dev-test-loop:\s*iteration\s*(\d+)\/(\d+)/, name: 'Dev-Test (Phase 6)' },
              { pattern: /review-loop:\s*iteration\s*(\d+)\/(\d+)/, name: 'Review (Phase 7)' },
              { pattern: /ci-fix-loop:\s*iteration\s*(\d+)\/(\d+)/, name: 'CI Fix (Phase 8)' },
              { pattern: /signoff-rejection-cycle:\s*(\d+)\/(\d+)/, name: 'Sign-off (Phase 10)' },
              { pattern: /deploy-loop:\s*iteration\s*(\d+)\/(\d+)/, name: 'Deploy (Phase 11)' },
              { pattern: /total-fix-attempts:\s*(\d+)\/(\d+)/, name: 'QA Bug Total (Phase 9)' },
            ];
            const loopText = loopSection[1];
            for (const { pattern, name } of breakerPatterns) {
              const m = loopText.match(pattern);
              if (m) {
                const current = parseInt(m[1]);
                const max = parseInt(m[2]);
                if (current >= max) {
                  console.log(`  ⚠️  CIRCUIT BREAKER TRIGGERED: ${name} — ${current}/${max} iterations reached!`);
                  console.log(`      Options: 1) Re-scope task  2) Split into smaller tasks  3) Cancel  4) Override (with justification)`);
                } else if (current >= max - 1) {
                  console.log(`  ⚠️  CIRCUIT BREAKER WARNING: ${name} — ${current}/${max} (LAST iteration before escalation)`);
                }
              }
            }

            // Check for circuit-breaker: true flag
            if (/circuit-breaker:\s*true/i.test(loopText)) {
              console.log(`  🛑 CIRCUIT BREAKER ACTIVE — loop escalated. Do NOT continue without user decision.`);
            }
          }
        }
