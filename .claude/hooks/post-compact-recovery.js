...
for (const file of files) {
  ...
  try {
    ...
    // Check for pre-compact snapshot (handle both field paths: snapshot.loop_state and snapshot.preserved.loop_state)
    const snapshotLoopState = snapshot && (snapshot.loop_state || (snapshot.preserved && snapshot.preserved.loop_state));
    if (snapshotLoopState) {
      console.log(`  Pre-compact snapshot with loop state available at .claude/reports/executions/`);
      console.log(`  Loop state from snapshot: ${snapshotLoopState.substring(0, 200)}`);
    }

    // Re-inject TASK_REGISTRY summary from snapshot (if available)
    const registrySnapshotDir = path.join(_projectRoot, 'logs', 'registry-snapshots');
    if (fs.existsSync(registrySnapshotDir)) {
      try {
        const registrySnaps = fs.readdirSync(registrySnapshotDir)
          .filter(f => f.startsWith('TASK_REGISTRY-'))
          .sort().reverse();
        if (registrySnaps.length > 0) {
          const regContent = fs.readFileSync(path.join(registrySnapshotDir, registrySnaps[0]), 'utf8');
          const todoCount = (regContent.match(/\|\s*TODO\s*\|/g) || []).length;
          const doneCount = (regContent.match(/\|\s*DONE\s*\|/g) || []).length;
          const inProgressCount = (regContent.match(/\|\s*IN_PROGRESS\s*\|/g) || []).length;
          console.log(`  REGISTRY: ${doneCount} done, ${inProgressCount} in-progress, ${todoCount} todo (from pre-compact snapshot)`);
        }
      } catch (e) { /* best effort */ }
    }

    // Re-inject TODO summary from snapshot (if available)
    if (fs.existsSync(registrySnapshotDir)) {
      try {
        const todoSnaps = fs.readdirSync(registrySnapshotDir)
          .filter(f => f.startsWith('TODO-'))
          .sort().reverse();
        if (todoSnaps.length > 0) {
          const todoContent = fs.readFileSync(path.join(registrySnapshotDir, todoSnaps[0]), 'utf8');
          const openItems = (todoContent.match(/^- \[ \] .+$/gm) || []).length;
          const doneItems = (todoContent.match(/^- \[x\] .+$/gm) || []).length;
          console.log(`  TODO: ${doneItems} done, ${openItems} open (from pre-compact snapshot)`);
        }
      } catch (e) { /* best effort */ }
    }
