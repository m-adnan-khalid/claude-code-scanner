...
process.stdin.on('end', () => {
  try {
    ...
    // Exception: always check Read, Agent, and Bash (high-token operations)
    const isHighToken = tool_name === 'Read' || tool_name === 'Agent' || tool_name === 'Bash';
    ...
    // Claude tokenizer: ~1.3 characters per token for English text, ~3-4 bytes per token for mixed content
    // Using /3 as balanced estimate (more accurate than /4 which underestimates by ~3x)
    ...
    const outputTokens = Math.ceil(outputBytes / 3);
    ...
    if (tool_name === 'Read') {
      ...
      state.agent_calls++;
      // Agent results can be large — use actual estimate, cap at 15000 to avoid outlier spikes
      state.estimated_tokens += Math.min(outputTokens, 15000);
    ...
    // Add conversation overhead (each tool call includes system+user+assistant framing)
    state.estimated_tokens += 600;
