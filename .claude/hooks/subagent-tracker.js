    ...
    if (fs.existsSync(tasksDir)) {
      ...
      for (const tf of taskFiles) {
        ...
        if (isTimeout) {
          ...
          // Auto-increment loop counter in task file
          try {
            const loopPatterns = [
              /dev-test-loop:\s*iteration\s*(\d+)\/(\d+)/,
              /review-loop:\s*iteration\s*(\d+)\/(\d+)/,
              /ci-fix-loop:\s*iteration\s*(\d+)\/(\d+)/,
              /deploy-loop:\s*iteration\s*(\d+)\/(\d+)/,
              /signoff-rejection-cycle:\s*(\d+)\/(\d+)/,
            ];
            let updated = content;
            for (const pattern of loopPatterns) {
              ...
              if (match) {
                ...
                updated = updated.replace(pattern, match[0].replace(`${current}/${max}`, `${newCount}/${max}`));
                fs.writeFileSync(taskPath, updated);
                console.log(`Loop counter incremented: ${current}/${max} → ${newCount}/${max} (agent timeout)`);
                if (newCount >= max) {
                  console.log(`⚠️  CIRCUIT BREAKER: Loop hit max ${max} iterations. Escalate to user.`);
                }
                break;
              }
            }
          } catch (e) { /* best effort — don't block on counter update */ }
