...
- **Verify Docs (3-step)**: (a) Read TECH_STACK.md to get exact framework versions chosen, (b) WebSearch `"<framework> <version> getting started / project structure docs"` for each, (c) only then scaffold. Never assume init commands, directory conventions, or config files from memory.
