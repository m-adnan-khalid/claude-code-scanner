...
**Step 1 — Read version:** Read the project's dependency file (`package.json`, `requirements.txt`, `pubspec.yaml`, `go.mod`, `Cargo.toml`, `build.gradle`, `Podfile`, `pom.xml`, `.csproj`) to get the EXACT installed version (e.g., Java 21, React 19.1, Django 5.2, Flutter 3.24).
**Step 2 — Search version-specific docs:** WebSearch `"<library/framework> <exact version> <API/method/class> documentation"` — e.g., `"Java 21 Stream.toList() documentation"`, `"React 19 useActionState documentation"`, `"Django 5.2 async views documentation"`. Never use docs for a different version.
**Step 3 — Write code:** Only after completing Steps 1-2 may you write code using that API. Cite the version you verified against in a brief code comment if the API is version-sensitive.
- If no version-specific docs found, use the closest version's docs and note the gap
- Never assume API signatures, method names, or patterns from training data alone
- Re-verify when using: deprecated APIs, newly added APIs, APIs that changed between major versions
